...
class Day02Tests(TestCase):
  ...
  def test_toboggan_policy(self):
    ...
    self.assertTrue(password_line_complies_with_policy(lines[0], policy_filter=toboggan_policy))
    self.assertFalse(password_line_complies_with_policy(lines[1], policy_filter=toboggan_policy))
    self.assertFalse(password_line_complies_with_policy(lines[2], policy_filter=toboggan_policy))
